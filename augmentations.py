...
def DataTransform(sample):
    ...
    print('jitter time: {:2.2f} seconds'.format(time.time() - start))
    start = time.time()
    x_scaling = scaling(sample, sigma=1.1) #sigma: [0.1, 2.]
    print('scaling time: {:2.2f} seconds'.format(time.time() - start))
    start = time.time()
    x_permutation = permutation(sample, max_segments=5, seg_mode="random") #max_segments: [3., 6.]
    print('permutation time: {:2.2f} seconds'.format(time.time() - start))
    start = time.time()
    x_rotation = rotation(sample)
    print('rotation time: {:2.2f} seconds'.format(time.time() - start))
    # start = time.time()
    x_magnitude_warp = magnitude_warp(sample, sigma=0.2, knot=4) #sigma: [0.1, 2.], knot: [3, 5] 0.2 4
    print('magnitude_warp time: {:2.2f} seconds'.format(time.time() - start))
    # start = time.time()
    x_time_warp = time_warp(sample, sigma=0.2, knot=4) #sigma: [0.01, 0.5], knot: [3, 5]
    print('time_warp time: {:2.2f} seconds'.format(time.time() - start))
    start = time.time()
    x_window_slice = window_slice(sample, reduce_ratio=0.9) #reduce_ratio: [0.95, 0.6]
    print('window_slice time: {:2.2f} seconds'.format(time.time() - start))

    return x_jitter, x_scaling, x_permutation, x_rotation,  x_magnitude_warp, x_time_warp, x_window_slice
